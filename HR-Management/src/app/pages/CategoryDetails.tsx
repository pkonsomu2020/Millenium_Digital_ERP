  ...
  const fetchCategoryData = async () => {
    try {
      ...
      const historyPromises = response.data.map((item: StockItem) =>
        api.getPurchaseHistory(item.id).then((res: { data: PurchaseHistory[] }) => ({
          itemId: item.id,
          data: res.data,
        }))
      );
              ...
              <Table>
                <TableHeader>
                  <TableRow className="bg-gray-100 dark:bg-gray-800">
                    <TableHead className="font-bold sticky left-0 bg-gray-100 dark:bg-gray-800 z-10 whitespace-nowrap">
                      ITEM NAME
                    </TableHead>
