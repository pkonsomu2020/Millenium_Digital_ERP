...
// Month(rowspan) | Date | item columns... | Comments
// TOTAL row (light blue) + TREND row (red) after each month, spacer between months
        ...
        <tbody>
          {months.map((month, mIdx) => {
            ...
            const prevShort = prevMonth
              ? new Date(prevMonth.key + "-01").toLocaleString("en-GB", { month: "short" }).toUpperCase()
              : "";
              ...
              <>
                {dateKeys.map((dateStr, dIdx) => (
                  <tr key={`${month.key}-${dateStr}`}>
                    {dIdx === 0 && (
                      <td rowSpan={dateKeys.length} className={`${S_MONTH} sticky left-0 z-10`}>
                        {shortMonth}
                      </td>
                    )}
                ...
                <tr key={`${month.key}-total`}>
                  ...
                  {items.map((item) => (
                    <td key={item.id} className={S_TOTAL}>{month.totals[item.id] ?? 0}</td>
                  ))}
...
// ─── Water Count ──────────────────────────────────────────────────────────────
// Delivery Date | Bottles Delivered | Total in Month
// Last row of each month highlighted with monthly total, spacer between months
            ...
            {months.map((month) => (
              <>
                ...
                <tr key={`${month.label}-spacer`}>
                  <td colSpan={3} className="h-3 bg-gray-100 dark:bg-[#111827] border-0"></td>
                </tr>
              </>
            ))}
...
// ─── Kitchen Stock ────────────────────────────────────────────────────────────
// # | Item Name | Current Qty | Purchased | Total | Notes
// Alternating rows, TOTAL ITEMS IN INVENTORY row at bottom
    ...
    <div className="p-4 sm:p-6 lg:p-8 space-y-5">
      <div className="flex items-center gap-3">
        <Button
          variant="outline"
          size="sm"
          onClick={() => navigate("/hr/dashboard/stock-management")}
