...
export function QtyCell({
  ...
  const calcClass = computed
    ...
    : "";
  return (
    <td className={`${className} ${accent} ${qtyCellClass(value, true)} ${calcClass}`} title={hint}>
      {fmtQtyDisplay(value)}
    </td>
  );
}
  ...
  if (!editable) {
    return (
      <p className="mx-3 sm:mx-4 lg:mx-6 mb-2 text-[11px] text-gray-500 dark:text-gray-400">
        Read-only view — entries cannot be changed here.
      </p>
    );
  }
  return (
      ...
      <div className="px-3 sm:px-6 py-3 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-start gap-2 sm:gap-3 min-w-0 flex-1">
          ...
          <div className="min-w-0 flex-1 pt-0.5">
            <h2 className="text-base sm:text-xl font-bold text-gray-900 dark:text-white leading-snug truncate">{title}</h2>
...
export function MonthTabs({
  ...
  return (
    <div className="px-3 sm:px-6 pt-3 pb-1">
      <div className="overflow-x-auto overscroll-x-contain [-webkit-overflow-scrolling:touch] scrollbar-thin pb-2" role="tablist" aria-label="Select month">
  ...
  const renderRow = (item: { id: string; item_name: string; unit: string }) => {
    ...
    const cell = (field: EntryField, accent: string) => {
      ...
      return canEdit ? (
        ...
        <QtyCell key={field} value={e[field]} accent={accent} computed hint={COMPUTED_FIELD_HINT[field]} />
      );
    };
  ...
  return (
    <div className="mb-6 sm:mb-8">
      ...
      <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:gap-3 px-3 sm:px-4 py-2.5 bg-[#4a90e2] text-white">
        ...
        <span className="text-[10px] sm:text-[11px] opacity-90 sm:ml-auto leading-snug">{p1Label} · {p2Label}</span>
      </div>
      <div className="relative">
        <div className="overflow-x-auto overscroll-x-contain [-webkit-overflow-scrolling:touch] touch-pan-x" aria-label="Stock register table — scroll horizontally for all columns">
        ...
        <div className="pointer-events-none absolute inset-y-0 right-0 w-6 bg-gradient-to-l from-white/90 to-transparent dark:from-gray-800/90 lg:hidden" aria-hidden />
      </div>
...
export function KitchenStockRegisterTable({
  ...
  return (
    <div className="mx-3 sm:mx-4 lg:mx-6 mb-6 rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden bg-white dark:bg-gray-800 shadow-sm">
      <div className="overflow-x-auto overscroll-x-contain [-webkit-overflow-scrolling:touch] touch-pan-x" aria-label="Kitchen stock table">
          ...
          {sortedItems.map((item) => {
            ...
            return (
              <tr key={item.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                ...
                <QtyCell value={closing} accent={REG.pClose} computed hint="Calculated: Opening + Bought − Broken/Lost" />
