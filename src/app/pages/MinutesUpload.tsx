...
export function MinutesUpload() {
  ...
  const handleDownload = async (url: string, filename: string) => {
    try {
      ...
      document.body.removeChild(link); window.URL.revokeObjectURL(dlUrl);
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 pt-4 sm:pt-6">
      <div className="mb-6">
        <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-white mb-2">Meeting Minutes</h1>
      ...
      {loading ? (
        ...
        <Card className="shadow-md dark:bg-gray-800 dark:border-gray-700"><CardContent className="p-8 text-center text-gray-500">No meeting minutes found</CardContent></Card>
      ) : (
        <div className="space-y-6">
          {sortedYearMonths.map((ym) => {
            ...
            return (
              <Card key={ym} className="shadow-md dark:bg-gray-800 dark:border-gray-700">
                <CardHeader className="bg-gray-100 dark:bg-gray-700 border-b dark:border-gray-600">
                  <CardTitle className="text-lg sm:text-xl text-[#374151] dark:text-white flex items-center gap-2"><Calendar className="w-5 h-5" />{monthName}</CardTitle>
                </CardHeader>
      ...
      <Dialog open={uploadDialogOpen} onOpenChange={setUploadDialogOpen}>
        <DialogContent className="sm:max-w-[500px] dark:bg-gray-800">
          <DialogHeader><DialogTitle className="text-xl font-semibold text-gray-900 dark:text-white">Upload Meeting Minutes</DialogTitle></DialogHeader>
          <div className="grid gap-4 py-4">
            <div className="grid gap-2"><Label htmlFor="file">Select File *</Label><Input id="file" type="file" onChange={handleFileSelect} accept=".pdf,.doc,.docx" className="dark:bg-gray-700 dark:border-gray-600" />{selectedFile && <p className="text-sm text-gray-600 dark:text-gray-400">Selected: {selectedFile.name} ({formatFileSize(selectedFile.size)})</p>}</div>
            <div className="grid gap-2"><Label htmlFor="meeting_title">Meeting Title *</Label><Input id="meeting_title" value={formData.meeting_title} onChange={(e) => setFormData({ ...formData, meeting_title: e.target.value })} placeholder="e.g., Q1 2026 Board Meeting" className="dark:bg-gray-700 dark:border-gray-600" /></div>
            <div className="grid gap-2"><Label htmlFor="meeting_date">Meeting Date *</Label><Input id="meeting_date" type="date" value={formData.meeting_date} onChange={(e) => setFormData({ ...formData, meeting_date: e.target.value })} className="dark:bg-gray-700 dark:border-gray-600" /></div>
            <div className="grid gap-2"><Label htmlFor="uploaded_by">Uploaded By *</Label><Input id="uploaded_by" value={formData.uploaded_by} onChange={(e) => setFormData({ ...formData, uploaded_by: e.target.value })} placeholder="Enter your name" className="dark:bg-gray-700 dark:border-gray-600" /></div>
            <div className="grid gap-2"><Label htmlFor="notes">Notes (Optional)</Label><Textarea id="notes" value={formData.notes} onChange={(e) => setFormData({ ...formData, notes: e.target.value })} placeholder="Add any notes about this meeting" className="dark:bg-gray-700 dark:border-gray-600" rows={3} /></div>
          </div>
          <DialogFooter>
      ...
      <Dialog open={editDialogOpen} onOpenChange={(v) => { setEditDialogOpen(v); if (!v) setReplaceFile(null); }}>
        <DialogContent className="sm:max-w-[500px] dark:bg-gray-800">
          ...
          <div className="grid gap-4 py-4">
            <div className="grid gap-2"><Label htmlFor="edit_title">Meeting Title *</Label><Input id="edit_title" value={editForm.meeting_title} onChange={(e) => setEditForm({ ...editForm, meeting_title: e.target.value })} className="dark:bg-gray-700 dark:border-gray-600" /></div>
            <div className="grid gap-2"><Label htmlFor="edit_date">Meeting Date *</Label><Input id="edit_date" type="date" value={editForm.meeting_date} onChange={(e) => setEditForm({ ...editForm, meeting_date: e.target.value })} className="dark:bg-gray-700 dark:border-gray-600" /></div>
            <div className="grid gap-2"><Label htmlFor="edit_uploaded_by">Uploaded By *</Label><Input id="edit_uploaded_by" value={editForm.uploaded_by} onChange={(e) => setEditForm({ ...editForm, uploaded_by: e.target.value })} className="dark:bg-gray-700 dark:border-gray-600" /></div>
            <div className="grid gap-2"><Label htmlFor="edit_notes">Notes</Label><Textarea id="edit_notes" value={editForm.notes} onChange={(e) => setEditForm({ ...editForm, notes: e.target.value })} className="dark:bg-gray-700 dark:border-gray-600" rows={3} /></div>
