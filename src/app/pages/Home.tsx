...
import { AlertTriangle, Clock, Calendar, Package, TrendingUp, Users, CheckCircle, XCircle, RefreshCw } from "lucide-react";
import {
...
export function Home() {
    ...
    return (
      <div className="p-8 flex flex-col items-center justify-center min-h-[50vh] gap-4">
        ...
        <button
          type="button"
          onClick={() => fetchStats()}
          className="px-4 py-2 rounded-lg bg-[#E76F51] text-white text-sm font-semibold hover:bg-[#D0593B]"
        >
          Try again
        </button>
      </div>
  ...
  const metricCards = [
    ...
    {
      title: "Pending Leave",
      value: leave.stats?.pending ?? 0,
      sub: `${leave.stats?.total ?? 0} total requests`,
      icon: Clock,
      gradient: "from-blue-500 to-blue-400",
    },
    {
      title: "Upcoming Meetings",
      value: meetings.stats?.upcoming ?? 0,
      sub: `${meetings.stats?.thisWeek ?? 0} this week`,
      icon: Calendar,
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 space-y-6">
            ...
            <Card key={m.title} className="overflow-hidden shadow-md hover:shadow-xl transition-all duration-300 dark:bg-gray-800 dark:border-gray-700 group">
              ...
              <CardContent className="pt-3 pb-3">
                <p className="text-xs text-gray-500 dark:text-gray-400 flex items-center gap-1">
                  {m.alert && <AlertTriangle className="w-3 h-3 text-amber-500" />}
                  {m.sub}
                </p>
              </CardContent>
            </Card>
      ...
      {/* Leave Status + Stock by Category */}
        ...
        <Card className="shadow-md dark:bg-gray-800 dark:border-gray-700">
          ...
          <CardContent>
            ...
            <div className="grid grid-cols-3 gap-2 mt-2">
              {[
                { label: "Pending", val: leave.stats?.pending ?? 0, color: "text-amber-500", bg: "bg-amber-50 dark:bg-amber-900/20" },
                { label: "Approved", val: leave.stats?.approved ?? 0, color: "text-emerald-500", bg: "bg-emerald-50 dark:bg-emerald-900/20" },
                { label: "Rejected", val: leave.stats?.rejected ?? 0, color: "text-red-500", bg: "bg-red-50 dark:bg-red-900/20" },
              ].map(item => (
            ...
            <div className="h-[260px]">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={stock.byCategory || []} margin={{ top: 5, right: 10, left: -20, bottom: 40 }}>
                  <defs>
                    {(stock.byCategory || []).map((_: any, i: number) => (
      ...
      {/* Purchase Trend + Meetings per Month */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">

        {/* Monthly Purchase Quantity Trend */}
        <Card className="shadow-md dark:bg-gray-800 dark:border-gray-700">
          <CardHeader className="pb-2">
            <CardTitle className="text-base text-[#374151] dark:text-white">Purchase Trend</CardTitle>
            <p className="text-xs text-gray-500 dark:text-gray-400">Total units purchased per month</p>
          </CardHeader>
          <CardContent>
            <div className="h-[260px]">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={purchases.monthly || []} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                  ...
                  <YAxis axisLine={false} tickLine={false} tick={{ fill: '#6B7280', fontSize: 11 }} allowDecimals={false} />
                  <Tooltip content={<CustomTooltip />} />
                  <Area type="monotone" dataKey="quantity" name="Units" stroke="#E76F51" strokeWidth={2.5} fill="url(#purchaseGrad)" dot={{ fill: '#E76F51', r: 4 }} activeDot={{ r: 6 }} />
                </AreaChart>
              </ResponsiveContainer>
            </div>
          </CardContent>
        ...
        <Card className="shadow-md dark:bg-gray-800 dark:border-gray-700">
          <CardHeader className="pb-2">
            <CardTitle className="text-base text-[#374151] dark:text-white">Meetings per Month</CardTitle>
            <p className="text-xs text-gray-500 dark:text-gray-400">Scheduled meetings over the last 6 months</p>
          </CardHeader>
          <CardContent>
            <div className="h-[260px]">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={meetings.perMonth || []} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                  <defs>
                    <linearGradient id="meetGrad" x1="0" y1="0" x2="1" y2="0">
      ...
      {/* Leave by Type + Low Stock Alerts */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">

        {/* Leave by Type */}
        <Card className="shadow-md dark:bg-gray-800 dark:border-gray-700">
          <CardHeader className="pb-2">
            <CardTitle className="text-base text-[#374151] dark:text-white">Leave by Type</CardTitle>
            <p className="text-xs text-gray-500 dark:text-gray-400">Distribution of leave request types</p>
          </CardHeader>
          <CardContent>
            <div className="h-[240px]">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={leave.byType || []} layout="vertical" margin={{ top: 5, right: 20, left: 10, bottom: 5 }}>
                  ...
                  <Tooltip content={<CustomTooltip />} />
                  <Bar dataKey="count" name="Requests" radius={[0, 6, 6, 0]} maxBarSize={22}>
                    {(leave.byType || []).map((_: any, i: number) => (
        ...
        <Card className="shadow-md dark:bg-gray-800 dark:border-gray-700">
          <CardHeader className="pb-2 flex flex-row items-center justify-between">
            <div>
              <CardTitle className="text-base text-[#374151] dark:text-white">Low Stock Alerts</CardTitle>
              <p className="text-xs text-gray-500 dark:text-gray-400">Items with 3 or fewer units</p>
            </div>
            {(stock.lowStock ?? 0) > 0 && (
              <Badge className="bg-red-100 text-red-600 dark:bg-red-900/30 dark:text-red-400 border-none">
                {stock.lowStock} alerts
              </Badge>
            )}
          </CardHeader>
              ...
              <Table>
                <TableHeader>
                  <TableRow className="bg-gray-50 dark:bg-gray-700/50">
                    <TableHead className="text-xs font-semibold text-gray-600 dark:text-gray-300">Item</TableHead>
                    <TableHead className="text-xs font-semibold text-gray-600 dark:text-gray-300">Category</TableHead>
                    <TableHead className="text-xs font-semibold text-gray-600 dark:text-gray-300 text-right">Qty</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
        ...
        <CardContent className="p-0">
          <div className="overflow-x-auto">
            <Table>
              <TableHeader>
                <TableRow className="bg-gray-50 dark:bg-gray-700/50">
                  <TableHead className="text-xs font-semibold text-gray-600 dark:text-gray-300">Date</TableHead>
                  <TableHead className="text-xs font-semibold text-gray-600 dark:text-gray-300">Item</TableHead>
                  <TableHead className="text-xs font-semibold text-gray-600 dark:text-gray-300 hidden sm:table-cell">Category</TableHead>
                  <TableHead className="text-xs font-semibold text-gray-600 dark:text-gray-300 text-right">Qty</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {(purchases.recent || []).length === 0 ? (
                  ...
                  <TableRow key={i} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                    ...
                    <TableCell className="text-xs text-gray-500 dark:text-gray-400 hidden sm:table-cell py-3">
                      <Badge variant="secondary" className="text-xs bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 border-none">{p.category}</Badge>
                    </TableCell>
