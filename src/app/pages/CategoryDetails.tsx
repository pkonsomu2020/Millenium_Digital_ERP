...
function AddPurchaseModal({ items, onClose, onSave }) {
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      await Promise.all(entries.map(([itemId, qty]) => api.addPurchaseHistory(itemId, { quantity: Number(qty), purchase_date: date, unit_price: 0, notes: "" })));
      toast.success("Purchase recorded"); onSave();
        ...
        <div className="space-y-2 max-h-64 overflow-y-auto pr-1">
          {items.map(item=>(
            <div key={item.id} className="flex items-center gap-3">
              ...
              <input type="number" min="0" placeholder="0" value={quantities[item.id]||""} onChange={e=>setQuantities(p=>({...p,[item.id]:e.target.value}))} className="w-24 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-[#111827] text-gray-900 dark:text-white px-2 py-1 text-sm text-center"/>
            </div>
          ))}
        </div>
        <div className="flex justify-end gap-2 pt-2">
          <Button variant="outline" size="sm" onClick={onClose} className="dark:border-gray-600 dark:text-gray-300">Cancel</Button>
          <Button size="sm" onClick={handleSave} disabled={saving} className="bg-[#D1131B] hover:bg-[#b01016] text-white">{saving?"Saving...":"Save"}</Button>
        </div>
      </div>
    </div>
  );
...
function AddWaterModal({ onClose, onSave }) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.success("Delivery recorded"); onSave();
    } catch(e) { toast.error(e.message||"Failed to save"); } finally { setSaving(false); }
  };
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="bg-white dark:bg-[#1a2235] rounded-2xl shadow-2xl w-full max-w-sm p-6 space-y-4">
        ...
        <div className="flex justify-end gap-2 pt-2">
          <Button variant="outline" size="sm" onClick={onClose} className="dark:border-gray-600 dark:text-gray-300">Cancel</Button>
          <Button size="sm" onClick={handleSave} disabled={saving} className="bg-[#4472C4] hover:bg-[#3560a8] text-white">{saving?"Saving...":"Save"}</Button>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="space-y-3">
      <div className="flex justify-end">
        <Button size="sm" onClick={onAddPurchase} className="flex items-center gap-1.5 bg-[#D1131B] hover:bg-[#b01016] text-white text-xs">
...
function WaterCountTable({ months, stats, onAddDelivery, onDeleteDelivery }) {
  return (
    <div className="space-y-5">
      ...
      <div className="flex justify-end">
        <Button size="sm" onClick={onAddDelivery} className="flex items-center gap-1.5 bg-[#4472C4] hover:bg-[#3560a8] text-white text-xs">
        ...
        <table className="border-collapse text-[11px]" style={{minWidth:"480px"}}>
          <thead>
            <tr>
              ...
              <th className={`${S_TH} min-w-[50px]`}></th>
            </tr>
            <tr>
                ...
                {month.deliveries.map((d,idx)=>{
                  ...
                  return(<tr key={d.id} className={rowCls}>
                    ...
                    <td className={`border border-gray-300 dark:border-gray-600 px-2 py-1 text-center ${rowCls}`}>
                      <button onClick={()=>onDeleteDelivery(d.id)} className="text-red-500 hover:text-red-700 transition-colors" title="Delete"><Trash2 className="w-3.5 h-3.5"/></button>
                    </td>
                  </tr>);
                })}
                <tr key={`${month.label}-spacer`}><td colSpan={4} className="h-3 bg-gray-100 dark:bg-[#111827] border-0"></td></tr>
              </>
            ))}
...
export function CategoryDetails() {
  ...
  const handleDeleteDelivery = async (id) => {
    ...
    try { await api.deleteWaterDelivery(id); toast.success("Delivery deleted"); load(); }
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 space-y-5">
      ...
      {loading ? (
        ...
        <>
          {cat==="Water Count"&&waterData&&<WaterCountTable months={waterData.months} stats={waterData.stats} onAddDelivery={()=>setShowAddWater(true)} onDeleteDelivery={handleDeleteDelivery}/>}
          ...
          {(cat==="Kitchen Essentials"||cat==="Washroom Essentials"||cat==="Snacks"||cat==="Other Purchases")&&monthlyData&&(
            <MonthlyPurchaseTable items={monthlyData.items} months={monthlyData.months} onAddPurchase={()=>setShowAddPurchase(true)}/>
          )}
        </>
      )}
      {showAddPurchase&&monthlyData&&<AddPurchaseModal items={monthlyData.items} onClose={()=>setShowAddPurchase(false)} onSave={()=>{setShowAddPurchase(false);load();}}/>}
      {showAddWater&&<AddWaterModal onClose={()=>setShowAddWater(false)} onSave={()=>{setShowAddWater(false);load();}}/>}
    </div>
  );
}