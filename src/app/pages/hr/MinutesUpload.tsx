...
interface MeetingMinutes { id: string; original_name: string; file_type: string; file_size: number; meeting_date: string; meeting_title: string; uploaded_by: string; upload_date: string; file_url: string; notes: string; }

export function HRMinutesUpload() {
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 pt-4 sm:pt-6">
      <div className="mb-6">
        ...
        <p className="text-gray-600 dark:text-gray-400">View meeting minutes for Millenium Solutions</p>
      </div>
      ...
      {loading ? <Card className="shadow-md dark:bg-gray-800 dark:border-gray-700"><CardContent className="p-8 text-center text-gray-500">Loading meeting minutes...</CardContent></Card> :
        filteredMinutes.length === 0 ? <Card className="shadow-md dark:bg-gray-800 dark:border-gray-700"><CardContent className="p-8 text-center text-gray-500">No meeting minutes found</CardContent></Card> : (
                          ...
                          <TableRow key={minute.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                            ...
                            <TableCell className="text-right">
                              <div className="flex gap-1 sm:gap-2 justify-end">
                                <Button variant="ghost" size="sm" className="text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 h-8 w-8 p-0" onClick={() => handleView(minute.file_url, minute.file_type)} title="View"><Eye className="w-3 h-3 sm:w-4 sm:h-4" /></Button>
                                <Button variant="ghost" size="sm" className="text-green-600 dark:text-green-400 hover:text-green-700 dark:hover:text-green-300 h-8 w-8 p-0" onClick={() => handleDownload(minute.file_url, minute.original_name)} title="Download"><Download className="w-3 h-3 sm:w-4 sm:h-4" /></Button>
                              </div>
                            </TableCell>
                          </TableRow>
                        ))}
