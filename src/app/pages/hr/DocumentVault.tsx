...
interface Document { id: string; original_name: string; file_type: string; file_size: number; category: string; uploaded_by: string; upload_date: string; file_url: string; notes: string; }
...
export function HRDocumentVault() {
  ...
  const categories = [{ name: "All Documents", count: documents.length }, ...CATEGORIES.map(cat => ({ name: cat, count: documents.filter(d => d.category === cat).length }))];
  const filteredDocs = documents.filter(doc => (doc.original_name.toLowerCase().includes(searchQuery.toLowerCase()) || doc.uploaded_by.toLowerCase().includes(searchQuery.toLowerCase())) && (selectedType === "All Documents" || doc.category === selectedType));

  const getTypeBadgeColor = (type: string) => ({ Proposal: "bg-green-500", Quotation: "bg-yellow-500", Contract: "bg-purple-500", Report: "bg-blue-500", Invoice: "bg-orange-500" }[type] || "bg-gray-500");
                ...
                <TableBody>
                  {filteredDocs.map((doc) => (
                    <TableRow key={doc.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                      <TableCell className="text-xs sm:text-sm"><div className="flex items-center gap-2"><FileText className="w-3 h-3 sm:w-4 sm:h-4 text-gray-500 dark:text-gray-400 flex-shrink-0" /><span className="text-gray-900 dark:text-white font-medium truncate max-w-[150px] sm:max-w-none">{doc.original_name}</span></div></TableCell>
                      <TableCell className="whitespace-nowrap"><Badge className={`${getTypeBadgeColor(doc.category)} hover:opacity-90 text-xs`}>{doc.category}</Badge></TableCell>
                      <TableCell className="text-gray-700 dark:text-gray-300 text-xs sm:text-sm whitespace-nowrap hidden sm:table-cell">{new Date(doc.upload_date).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })}</TableCell>
