...
const CATEGORY_ORDER = ["Kitchen Essentials","Washroom Essentials","Water Count","Kitchen Stock","Snacks","Other Purchases"];
...
const CATEGORY_ICONS: Record<string, any> = {
  "Kitchen Essentials": Utensils, "Washroom Essentials": Droplets,
  "Water Count": Waves, "Kitchen Stock": ShoppingBag, "Snacks": Apple, "Other Purchases": Package,
};
...
export function HRStockManagement() {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const [stockResponse, statsResponse] = await Promise.all([api.getAllStock(), api.getStockStats()]);
  ...
  const groupedItems = CATEGORY_ORDER.reduce((acc, category) => {
    const items = stockItems.filter(item =>
      ...
      (item.item_name.toLowerCase().includes(searchQuery.toLowerCase()) || item.category.toLowerCase().includes(searchQuery.toLowerCase()))
    );
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 space-y-6">
      ...
      <div className="relative">
        ...
        <Input placeholder="Search items or categories..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-9 dark:bg-gray-800 dark:border-gray-600 dark:text-white" />
      </div>
      {loading ? <div className="text-center py-16 text-gray-400">Loading...</div> :
        Object.keys(groupedItems).length === 0 ? <div className="text-center py-16 text-gray-400">No items found</div> : (
        <div className="space-y-4">
          {Object.entries(groupedItems).map(([category, items]) => {
            const totalQty = items.reduce((s, i) => s + (i.current_quantity || 0), 0);
            const CatIcon = CATEGORY_ICONS[category] || Package;
            return (
              <div key={category} className="rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-[#1a2235] shadow-md overflow-hidden">
                <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 dark:border-gray-700/60">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="flex items-center justify-center w-6 h-6 rounded-full bg-[#D1131B] text-white text-xs font-bold">{items.length}</span>
                  </div>
                  <div className="flex items-center gap-4">
                    <span className="text-sm text-gray-500 dark:text-gray-400 hidden sm:block">Total Stock: <span className="font-bold text-gray-800 dark:text-white">{totalQty}</span> units</span>
                    <Button variant="ghost" size="sm" onClick={() => navigate(`/hr/dashboard/stock-management/${encodeURIComponent(category)}`)} className="flex items-center gap-1.5 text-[#D1131B] hover:text-[#D1131B] hover:bg-red-50 dark:hover:bg-red-900/20 font-medium text-sm">
                ...
                <div className="p-4 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
                  {items.map((item) => {
                    ...
                    return (
                      <div key={item.id} className={`relative flex items-center gap-3 px-3 py-3 rounded-xl border transition-all ${isLow ? "border-[#D1131B]/60 bg-red-50/40 dark:bg-red-900/10 dark:border-[#D1131B]/50" : "border-gray-200 dark:border-gray-600/60 bg-gray-50 dark:bg-[#111827]/60"}`}>
                        <div className={`shrink-0 w-8 h-8 rounded-lg flex items-center justify-center ${isLow ? "bg-[#D1131B]/10 dark:bg-[#D1131B]/20" : "bg-gray-200/70 dark:bg-gray-700/60"}`}>
                        ...
                        <div className="min-w-0">
                          ...
                          <p className={`text-xs font-bold mt-0.5 ${isLow ? "text-[#D1131B]" : "text-gray-500 dark:text-gray-400"}`}>{item.current_quantity} {item.unit}</p>
                          {isLow && <div className="absolute bottom-0 left-3 right-3 h-0.5 rounded-full bg-[#D1131B]/70" />}
                        </div>
                      </div>
                    );
                  })}
